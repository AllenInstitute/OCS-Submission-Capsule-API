...
study_list(facs_well_id, studies) as (
    select 
        fwstdy.facs_well_id, 
        array_to_string(array_agg(stdy.name), ', ') as studies
...
donor_list (
    ...
    inj_table(donor_id, inj_roi, inj_method, inj_materials) as (
        ...
        select distinct donor_id, inj_roi, inj_method, string_agg(inj_materials, '_and_') as inj_materials
        from inj_pre_agg
    ...
    select 
        di.fw_id, 
        string_agg(coalesce(di.donor, 'NULL'), ';' order by di.donor) as donor_name,
        string_agg(coalesce(di.cell_prep_sample_name, 'NULL'), ';' order by di.donor) as cell_prep_sample_name,
        string_agg(coalesce(di.full_genotype, 'NULL'), ';' order by di.donor) as full_genotype,
        string_agg(coalesce(di.external_donor_name, 'NULL'), ';' order by di.donor) as external_donor_name,
        string_agg(coalesce(di.age, 'NULL'), ';' order by di.donor) as age,
        string_agg(coalesce(di.sex, 'NULL'), ';' order by di.donor) as sex,
        string_agg(coalesce(di.species, 'NULL'), ';' order by di.donor) as species,
        string_agg(coalesce(di.organism, 'NULL'), ';' order by di.donor) as organism,
        string_agg(coalesce(dr.name, 'NULL'), ';' order by di.donor) as cre_line,
        string_agg(coalesce(rp.name, 'NULL'), ';' order by di.donor) as reporter,
        string_agg(coalesce(mc.name, 'NULL'), ';' order by di.donor) as medical_conditions,
        string_agg(coalesce(inj.inj_roi, 'NULL'), ';' order by di.donor) as injection_roi,
        string_agg(coalesce(inj.inj_method, 'NULL'), ';' order by di.donor) as injection_method,
        string_agg(coalesce(inj.inj_materials, 'NULL'), ';' order by di.donor) as injection_materials
    from donor_items di
...
(
select distinct cell.id, st.acronym from specimens cell
...
order by armd.expc_name_from_vendor;