...
WITH
	core_cte AS (
		SELECT
			...
			fwrc.facs_well_id AS sample_id,
		...
		JOIN facs_wells_rseq_experiment_components fwrc ON fwrc.rseq_experiment_component_id = rec.id
		JOIN rna_amplification_inputs rai ON rai.sample_id = fwrc.facs_well_id
		...
		JOIN facs_well_templates fwt ON fwt.id = fwrc.facs_well_id
...
FROM core_cte cc
JOIN analysis_run_metadata_cte armc
	ON armc.rseq_experiment_component_id = cc.rseq_experiment_component_id
	AND armc.sample_id = cc.sample_id
...
ORDER BY cc.exp_component_vendor_name, cc.sample_name;
